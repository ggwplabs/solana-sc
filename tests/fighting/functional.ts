...
describe("Fighting functional tests", () => {
  ...
  before(async () => {
    ...
    await fighting.methods.initialize(fixture.updateAuth.publicKey, fixture.fighting.validator.publicKey, new anchor.BN(afkTimeout), rewardCoefficient, gpassDailyRewardCoefficient, royalty)
      .accounts({
    ...
    assert.ok(fightingSettingsData.validator.equals(fixture.fighting.validator.publicKey));
    ...
    gameInfo = findProgramAddressSync(
      [
        ...
        new anchor.BN(gameId).toArrayLike(Buffer, "le", 4),
      ],
      fighting.programId,
    )[0];
  });
  ...
  it("Try to finalize game with invalid validator", async () => {
    await assert.rejects(fighting.methods.finalizeGame(gameId, { win: {} }, [])
      .accounts({
        user: fixture.user.kp.publicKey,
      ...
      (e: AnchorError) => {
        assert.ok(e.error !== undefined);
        assert.strictEqual(e.error.errorCode.code, "InvalidValidator");
        assert.strictEqual(e.error.errorCode.number, 6009);
        assert.strictEqual(e.error.errorMessage, "Invalid validator");
        return true;
      });
  });
  ...
  it("Try to finalize game with user not in game", async () => {
    await assert.rejects(fighting.methods.finalizeGame(gameId, { win: {} }, [])
      .accounts({
        user: fixture.user.kp.publicKey,
        validator: fixture.fighting.validator.publicKey,
        userInfo: fixture.user.info,
        userGgwpWallet: fixture.user.ggwpWallet,
        gameInfo: gameInfo,
        accumulativeFund: fixture.fighting.accumulativeFund,
      ...
      .signers([fixture.user.kp, fixture.fighting.validator])
      .rpc(),
      (e: AnchorError) => {
        assert.ok(e.error !== undefined);
  ...
  it("Try to finalize game with invalid actions log size", async () => {
    ...
    await assert.rejects(fighting.methods.finalizeGame(gameId, { win: {} }, actions)
      .accounts({
        user: fixture.user.kp.publicKey,
        validator: fixture.fighting.validator.publicKey,
        userInfo: fixture.user.info,
  ...
  it("Finalize game", async () => {
    let validatorBalanceBefore = await fighting.provider.connection.getBalance(fixture.fighting.validator.publicKey);
    ...
    await fighting.methods.finalizeGame(gameId, { win: {} }, actions)
      .accounts({
        user: fixture.user.kp.publicKey,
        validator: fixture.fighting.validator.publicKey,
        userInfo: fixture.user.info,
    ...
    assert.equal(gameInfoData.id, gameId);
    ...
    let validatorBalance = await fighting.provider.connection.getBalance(fixture.fighting.validator.publicKey);
